const getFileNodes = dictionary =>
  Object.keys(dictionary).filter(
    node => dictionary[node].category === "data_file"
  );

const getNodeTypes = dictionary =>
  Object.keys(dictionary).filter(node => node.charAt(0) !== "_");
...
const submission = (state = {}, action) => {
  switch (action.type) {
    ...
    case "RECEIVE_DICTIONARY":
      return {
        ...state,
        dictionaryName: action.data.dictionaryName,
        dictionary: getDictionaryWithExcludeSystemProperties(action.data.dictionary),
        nodeTypes: getNodeTypes(action.data.dictionary),
        file_nodes: getFileNodes(action.data.dictionary)
      };
    case "RECEIVE_DICTIONARY_LIST":
      return { ...state, dictList: action.data || [] };
